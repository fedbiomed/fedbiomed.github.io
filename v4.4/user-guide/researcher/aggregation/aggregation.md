...
[API reference](/developer/api/researcher/aggregators/#fedbiomed.researcher.aggregators.Scaffold).
...
Please note that:
 ...
 - `SCAFFOLD` **requires** using the `num_updates` training argument to control the number of [training iterations](/user-guide/researcher/experiment/#controlling-the-number-of-training-loop-iterations). Using only `epochs` will raise an error.
...
[aggregate method](/developer/api/researcher/aggregators/#fedbiomed.researcher.aggregators.fedavg.FedAverage.aggregate). 
...
[Experiment]((/user-guide/researcher/experiment)) as an argument.

...
* [deploy second node on the same machine](../deployment-vpn-node2/#deploy-a-second-node-instance-on-the-same-node-machine)
...
* for [the server side](../deployment-vpn-secagg/#configure-secure-aggregation-for-the-server-side)
* for [the node side](../deployment-vpn-secagg/#configure-secure-aggregation-for-the-node-side)
...
* if node GUI is launched, it can be used to share datasets. On the node side machine, connect to `http://localhost:8484`
...
* [use second node on the same machine](../deployment-vpn-node2/#use-the-second-node-instance-on-the-same-node-machine)
